...
export default function App(){
  ...
  const addEstItem=()=>setEstItems(prev=>[...prev,{id:Date.now(),name:"",spec:"",qty:"1",unit:"式",unitPrice:"",total:0}]);
  ...
  const saveEst=()=>{
    ...
    const records=estDates.map(date=>({
      id:Date.now()+Math.random(),
      date,
      estJobName:estJobName.trim(),
      estClient:estClient.trim(),
      estBilling:estBilling,
      estItems:estItems.filter(it=>it.name.trim()),
      estSubtotal,
      estNote:estNote.trim(),
      type:"estimate"
    }));
    const updated=editEst
      ?data.workRecords.map(x=>x.id===editEst.id?records[0]:x)
      :[...records,...(data.workRecords||[])];
    save({...data,workRecords:updated});
    setEstJobName("");setEstJobInput("");setEstClient("");setEstBilling("");setEstItems([{id:1,name:"",spec:"",qty:"1",unit:"式",unitPrice:"",taxRate:"10"}]);
  ...
  const startEditEst=r=>{
    ...
    setEstItems(r.estItems?.length>0?r.estItems:[{id:1,name:"",spec:"",qty:"1",unit:"式",unitPrice:"",taxRate:"10"}]);
    setEstDates([r.date]);setEstNote(r.estNote||"");setShowEstForm(true);
  };
    ...
    <div style={{fontFamily:"'Hiragino Sans','Hiragino Kaku Gothic ProN',YuGothic,sans-serif",background:"#F7F6F3",minHeight:"100vh",color:"#2E2B27",maxWidth:460,margin:"0 auto"}}>
      ...
      {tab==="cost"&&<div style={{padding:"14px 14px 100px"}}>
        ...
        <div style={{display:"flex",gap:16,borderBottom:"1px solid #E8E4DC",marginBottom:12}}>
          {[["records","記録"],["gantt","ガント"],["summary","集計"]].map(([k,l])=>(
        ...
        {costSubTab==="records"&&<>
          ...
          {showEstForm&&<Card style={{marginBottom:12}} className="fade">
            ...
            {/* 月カレンダー マルチセレクト */}
            <div style={{marginBottom:10}}>
              <div style={{fontSize:11,color:"#C0BAB0",marginBottom:6}}>日付（複数選択可）</div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
              <div>
                <div style={{fontSize:11,color:"#C0BAB0",marginBottom:4}}>請求額（税抜）</div>
                <Inp type="number" value={estBilling} onChange={e=>setEstBilling(e.target.value)} placeholder="0"/>
              </div>
              <div>
                <div style={{fontSize:11,color:"#C0BAB0",marginBottom:4}}>原価合計</div>
                <div style={{background:"#F4F2EE",border:"1px solid #E8E4DC",borderRadius:9,padding:"10px 12px",fontSize:13,color:"#8A8070"}}>¥{yen(estSubtotal)}</div>
              </div>
            </div>

            {/* 明細テーブル */}
            <div style={{marginBottom:8}}>
              <div style={{fontSize:11,color:"#C0BAB0",marginBottom:6}}>明細</div>
              <div style={{background:"#F8F7F4",borderRadius:8,overflow:"hidden",border:"1px solid #E8E4DC"}}>
                <div style={{display:"grid",gridTemplateColumns:"2fr 1.5fr 0.8fr 0.5fr 1.2fr 1fr 28px",gap:0,padding:"6px 8px",background:"#F0EDE7"}}>
                  {["名称","仕様","数量","単位","単価","課税",""].map((h,i)=><div key={i} style={{fontSize:10,color:"#9E9890",fontWeight:500}}>{h}</div>)}
                </div>
                {estItems.map((it)=>(
                  <div key={it.id} style={{display:"grid",gridTemplateColumns:"2fr 1.5fr 0.8fr 0.5fr 1.2fr 1fr 28px",gap:2,padding:"6px 8px",borderTop:"1px solid #E8E4DC",alignItems:"center"}}>
                    ...
                    <input value={it.spec} onChange={e=>updateEstItem(it.id,"spec",e.target.value)} placeholder="仕様" style={{background:"transparent",border:"none",fontSize:12,color:"#2E2B27",fontFamily:"inherit",outline:"none",width:"100%"}}/>
                    ...
                    {estItems.length>1&&<button onClick={()=>delEstItem(it.id)} style={{background:"none",border:"none",color:"#D8D3CA",fontSize:16,cursor:"pointer",padding:0}}>×</button>}
                  </div>
                ))}
                <div style={{padding:"6px 8px",borderTop:"1px solid #E8E4DC"}}>
                  <button onClick={addEstItem} style={{background:"none",border:"none",color:"#8A99B0",fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>＋ 行を追加</button>
                </div>
              </div>
            </div>

            {/* 税額サマリー */}
            <div style={{background:"#F4F2EE",borderRadius:8,padding:"10px 12px",marginBottom:10}}>
              {(()=>{
                const t10=estItems.filter(it=>it.taxRate==="10"||!it.taxRate).reduce((s,it)=>s+parseFloat(it.qty||0)*parseFloat(it.unitPrice||0),0);
                const t8=estItems.filter(it=>it.taxRate==="8").reduce((s,it)=>s+parseFloat(it.qty||0)*parseFloat(it.unitPrice||0),0);
                const t0=estItems.filter(it=>it.taxRate==="0").reduce((s,it)=>s+parseFloat(it.qty||0)*parseFloat(it.unitPrice||0),0);
                const tax10=Math.round(t10*0.1);
                const tax8=Math.round(t8*0.08);
                const subtotal=t10+t8+t0;
                const totalTax=tax10+tax8;
                const grandTotal=subtotal+totalTax;
                return(<>
                  {t10>0&&<div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"#9E9890",marginBottom:2}}><span>10%対象</span><span>¥{yen(t10)} (税 ¥{yen(tax10)})</span></div>}
                  {t8>0&&<div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"#9E9890",marginBottom:2}}><span>8%対象</span><span>¥{yen(t8)} (税 ¥{yen(tax8)})</span></div>}
                  {t0>0&&<div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"#9E9890",marginBottom:2}}><span>非課税</span><span>¥{yen(t0)}</span></div>}
                  <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"#9E9890",marginBottom:4,paddingTop:4,borderTop:"1px solid #E8E4DC"}}><span>消費税合計</span><span>¥{yen(totalTax)}</span></div>
                  <div style={{display:"flex",justifyContent:"space-between",fontSize:15,fontWeight:700,color:"#2E2B27"}}><span>税込合計</span><span>¥{yen(grandTotal)}</span></div>
                </>);
              })()}
            </div>
          ...
          {!showEstForm&&<button onClick={()=>{setShowEstForm(true);setEditEst(null);setEstJobName("");setEstJobInput("");setEstClient("");setEstItems([{id:1,name:"",spec:"",qty:"1",unit:"式",unitPrice:"",taxRate:"10"}]);setEstDates([isoDay(new Date())]);setEstBilling("");setEstNote("");setEstShowHistory(false);}} style={{width:"100%",padding:"11px",background:"transparent",border:"1.5px dashed #DDD8D0",borderRadius:10,color:"#C0BAB0",fontSize:13,marginBottom:12,cursor:"pointer",fontFamily:"inherit"}}>＋ 原価を追加</button>}
              ...
              return(<div key={r.id} className="row" style={{background:"#fff",border:"1px solid #EAE7E1",borderRadius:10,padding:"11px 14px",marginBottom:6}}>
                <div style={{display:"flex",alignItems:"flex-start",gap:8}}>
                  <div style={{flex:1}}>
                    ...
                    {r.estClient&&<div style={{fontSize:11,color:"#8A99B0",marginBottom:3}}>元請：{r.estClient}</div>}
                    ...
                    {items.filter(it=>it.name).map((it,i)=>(
                      <div key={i} style={{fontSize:11,color:"#6E6A63",marginBottom:2,display:"flex",justifyContent:"space-between"}}>
                        <span>{it.name}{it.spec?" / "+it.spec:""} {it.qty}{it.unit}</span>
                        <span style={{color:it.taxRate==="0"?"#9E9890":"#4A4740"}}>¥{yen(parseFloat(it.qty||0)*parseFloat(it.unitPrice||0))}{it.taxRate==="0"?" (非課税)":""}</span>
                      </div>
                    ))}
                    <div style={{marginTop:6,paddingTop:6,borderTop:"1px solid #F0EDE7"}}>
                      <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"#9E9890",marginBottom:2}}><span>税抜</span><span>¥{yen(sub)}</span></div>
                      <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"#9E9890",marginBottom:2}}><span>消費税</span><span>¥{yen(tax)}</span></div>
                      <div style={{display:"flex",justifyContent:"space-between",fontSize:13,fontWeight:700,color:"#2E2B27"}}><span>税込合計（原価）</span><span style={{color:"#C49A5A"}}>¥{yen(total)}</span></div>
                      {r.estBilling&&<div style={{display:"flex",justifyContent:"space-between",fontSize:13,fontWeight:700,color:"#7CA37A",marginTop:2}}><span>請求額</span><span>¥{yen(parseInt(r.estBilling||0))}</span></div>}
                      {r.estBilling&&<div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:"#8A99B0",marginTop:2}}><span>粗利</span><span style={{fontWeight:600}}>¥{yen(parseInt(r.estBilling||0)-total)}</span></div>}
                    </div>
                    {r.estNote&&<div style={{fontSize:11,color:"#9E9890",marginTop:4}}>{r.estNote}</div>}
                  </div>
