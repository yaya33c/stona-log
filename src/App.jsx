...
export default function App(){
  ...
  const [jIncome,setJIncome]=useState("");
  const [jOutgo,setJOutgo]=useState("");
  ...
  const saveJob=()=>{
    ...
    const j={id:editJob?.id||Date.now(),createdAt:editJob?.createdAt||new Date().toISOString(),name:jName.trim(),client:jClient.trim(),billing:parseInt(jBilling)||0,status:jStatus,startDate:jStartDate,endDate:jEndDate,income:parseInt(jIncome)||0,outgo:parseInt(jOutgo)||0};
    save({...data,jobs:editJob?data.jobs.map(x=>x.id===editJob.id?j:x):[j,...data.jobs]});
    setJName("");setJClient("");setJBilling("");setJStatus("確定");setJIncome("");setJOutgo("");setShowJobForm(false);setEditJob(null);
  };
  ...
  const LogItem=({item})=>(
    <div className="row fade" style={{padding:"10px 0",borderBottom:"1px solid #F0EDE7"}}>
      ...
      {item._t==="post"&&(item.mode==="work"||item.mode==="hobby")&&(
        <div style={{display:"flex",gap:6,marginTop:8}}>
          <button onClick={()=>{
            setWhyEvent(item.text);setWhy1("");setWhy2("");setWhy3("");setShowWhy(true);
            setTimeout(()=>whyRef.current?.scrollIntoView({behavior:"smooth",block:"start"}),50);
          }} style={{fontSize:11,padding:"4px 10px",borderRadius:16,border:"1px solid #E8E4DC",background:"#F4F2EE",color:"#8A8070",cursor:"pointer",fontFamily:"inherit"}}>
            🔍 なぜ深掘り
          </button>
          <button onClick={()=>{
            setThoughtPerspective("");setThoughtText("");setShowThought(true);
            setTimeout(()=>thoughtRef.current?.scrollIntoView({behavior:"smooth",block:"start"}),50);
          }} style={{fontSize:11,padding:"4px 10px",borderRadius:16,border:"1px solid #E8E4DC",background:"#F4F2EE",color:"#8A8070",cursor:"pointer",fontFamily:"inherit"}}>
            💡 視点を変える
          </button>
        </div>
        ...
        <Card style={{marginBottom:12}}>
          {showJobForm
            ?<div className="fade">
              ...
              <div style={{fontSize:11,color:"#C49A5A",fontWeight:600,marginBottom:6,marginTop:4}}>予想金額</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:12}}>
                <div>
                  <div style={{fontSize:10,color:"#C0BAB0",marginBottom:4}}>入金額（予想）</div>
                  <Inp type="number" value={jIncome} onChange={e=>setJIncome(e.target.value)} placeholder="0"/>
                </div>
                <div>
                  <div style={{fontSize:10,color:"#C0BAB0",marginBottom:4}}>出金額（予想）</div>
                  <Inp type="number" value={jOutgo} onChange={e=>setJOutgo(e.target.value)} placeholder="0"/>
                </div>
                <div>
                  <div style={{fontSize:10,color:"#C0BAB0",marginBottom:4}}>粗利（自動）</div>
                  <div style={{background:"#F4F2EE",border:"1px solid #E8E4DC",borderRadius:9,padding:"10px 12px",fontSize:12,color:(parseInt(jIncome)||0)-(parseInt(jOutgo)||0)>=0?"#7CA37A":"#E07070",fontWeight:600}}>
                    ¥{yen((parseInt(jIncome)||0)-(parseInt(jOutgo)||0))}
                  </div>
                </div>
              </div>
              <div style={{display:"flex",gap:8}}>
                <Btn onClick={saveJob} variant="primary" style={{flex:1}}>{editJob?"更新":"追加"}</Btn>
                <Btn onClick={()=>{setShowJobForm(false);setEditJob(null);setJName("");setJClient("");setJStartDate("");setJEndDate("");setJStatus("確定");setJIncome("");setJOutgo("");}} variant="ghost">キャンセル</Btn>
              </div>
            </div>
                ...
                <div style={{minWidth:NAME_W+STATUS_W+12*CELL_W}}>
                  ...
                  {allJobs.length>0&&<>
                    ...
                    {(()=>{
                      ...
                      const hasAny=allJobs.some(j=>j.income||j.outgo);
                      if(!hasAny)return null;
                      return(
                        <div style={{borderTop:"1px solid #F0EDE7",padding:"10px 12px"}}>
                          <div style={{fontSize:10,color:"#C0BAB0",fontWeight:700,marginBottom:8}}>月別合計（予想）</div>
                          <div style={{overflowX:"auto"}}>
                            <table style={{width:"100%",borderCollapse:"collapse",fontSize:11,minWidth:360}}>
                              <thead>
                                <tr style={{background:"#F0EDE7"}}>
                                  <th style={{padding:"5px 8px",textAlign:"left",color:"#5A5550",fontWeight:700,whiteSpace:"nowrap",border:"1px solid #EAE7E1"}}>月</th>
                                  <th style={{padding:"5px 8px",textAlign:"right",color:"#8A99B0",fontWeight:700,border:"1px solid #EAE7E1"}}>入金</th>
                                  <th style={{padding:"5px 8px",textAlign:"right",color:"#C49A5A",fontWeight:700,border:"1px solid #EAE7E1"}}>出金</th>
                                  <th style={{padding:"5px 8px",textAlign:"right",color:"#7CA37A",fontWeight:700,border:"1px solid #EAE7E1"}}>粗利</th>
                                </tr>
                              </thead>
                              <tbody>
                                {summaryMonths.map(({key,label})=>{
                                  // その月に工期が重なる案件を集計
                                  const [yr2,mo2]=[parseInt(key.split("-")[0]),parseInt(key.split("-")[1])-1];
                                  const mStart=new Date(yr2,mo2,1);
                                  const mEnd=new Date(yr2,mo2+1,0);
                                  ...
                                  const totalIncome=activeJobs.reduce((s,j)=>s+(j.income||0),0);
                                  const totalOutgo=activeJobs.reduce((s,j)=>s+(j.outgo||0),0);
                                  const totalProfit=totalIncome-totalOutgo;
                                  return(
                                    <tr key={key} style={{background:"#fff"}}>
                                      <td style={{padding:"5px 8px",border:"1px solid #F0EDE7",color:"#2E2B27",fontWeight:600}}>{label}</td>
                                      <td style={{padding:"5px 8px",border:"1px solid #F0EDE7",textAlign:"right",color:"#8A99B0"}}>¥{yen(totalIncome)}</td>
                                      <td style={{padding:"5px 8px",border:"1px solid #F0EDE7",textAlign:"right",color:"#C49A5A"}}>¥{yen(totalOutgo)}</td>
                                      <td style={{padding:"5px 8px",border:"1px solid #F0EDE7",textAlign:"right",color:totalProfit>=0?"#7CA37A":"#E07070",fontWeight:600}}>¥{yen(totalProfit)}</td>
                                    </tr>
                                  );
                                })}
                              </tbody>
