...
export default function App() {
  ...
  const searchBooks = async () => {
    ...
    try {
      const res = await fetch(`https://www.googleapis.com/books/v1/volumes?q=${encodeURIComponent(bookQuery)}&langRestrict=ja&maxResults=5`);
      const json = await res.json();
      setBookResults((json.items||[]).map(i=>({
        id:i.id, title:i.volumeInfo.title||"",
        authors:(i.volumeInfo.authors||[]).join(", "),
        thumbnail:i.volumeInfo.imageLinks?.thumbnail?.replace("http:","https:") || "",
      })));
    } catch {}
  ...
  const searchPlace = async () => {
    ...
    try {
      const res = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(placeName)}&limit=4&accept-language=ja`);
      const json = await res.json();
      setPlaceResults(json.map(p=>({name:p.display_name.split(",")[0],fullName:p.display_name,lat:parseFloat(p.lat),lon:parseFloat(p.lon)})));
    } catch {}
...
【営業・案件のヒント】
- 気づきや提案を1〜2点`,
          messages:[{role:"user",content:`直近の仕事メモ：\n\n${memos}`}]
        })
      });
      const json = await res.json();
  ...
  const css = `
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    ::-webkit-scrollbar-thumb{background:#E2DDD5;border-radius:2px}
    .fade{animation:fi 0.2s ease}
    @keyframes fi{from{opacity:0;transform:translateY(4px)}to{opacity:1}}
    .row:hover .x{opacity:1!important}
    textarea{resize:none}
    button{cursor:pointer;font-family:inherit}
    .hover-bg:hover{background:#F4F2EE!important}
  `;
  ...
  return (
    <div style={{fontFamily:"'Hiragino Sans','Hiragino Kaku Gothic ProN',YuGothic,sans-serif",background:"#F7F6F3",minHeight:"100vh",color:"#2E2B27",maxWidth:460,margin:"0 auto"}}>
      ...
      <div style={{display:"flex",padding:"14px 20px 0",gap:20,borderBottom:"1px solid #E8E4DC"}}>
        {[["log","記録"],["goal","目標"],["ai","AI分析"]].map(([k,l])=>(
